...
__declspec(align(128)) unsigned long r4300emu = 0, interpcore = 0;
int no_audio_delay = 0;
int no_compiled_jump = 0;
__declspec(align(128)) int stop, llbit;
__declspec(align(128)) long long int reg[36];
__declspec(align(128)) unsigned long reg_cop0[32];
__declspec(align(128)) long local_rs32, local_rt32;
__declspec(align(128)) unsigned long jump_target;
__declspec(align(128)) float * reg_cop1_simple[32];
__declspec(align(128)) double * reg_cop1_double[32];
__declspec(align(128)) long long int reg_cop1_fgr_64[32];
__declspec(align(128)) long FCR0, FCR31;
__declspec(align(128)) tlb tlb_e[32];
__declspec(align(128)) unsigned long delay_slot, skip_jump = 0, dyna_interp = 0, last_addr;
__declspec(align(128)) unsigned long long int debug_count = 0;
__declspec(align(128)) unsigned int next_interupt, CIC_Chip;
__declspec(align(128)) precomp_instr * PC;
__declspec(align(4096)) char invalid_code[0x100000];
...
__inline unsigned long update_invalid_addr(unsigned long addr)
{
...
void go()
{
	...
	{ char _dbg[128]; sprintf(_dbg, "[go] r4300emu=%d", r4300emu); DLOG(_dbg); }
	...
	{
		PC = malloc(sizeof (precomp_instr));
		memset(PC, 0, sizeof(precomp_instr));
		r4300emu = 0;
		interpcore = 1;
		DLOG("[go] calling prefetch then pure_interpreter...");
		prefetch();
		pure_interpreter();
		DLOG("[go] pure_interpreter returned");
		r4300emu = 2;
	}
	else
	{
...
void r4300_reset_soft(void)
{
	...
	sprintf(_dbg, "[r4300] r4300_reset_soft rom=%p sp_dmem=%p", (void*)rom, (void*)SP_DMEM);
	DLOG(_dbg);
	if (rom) {
		sprintf(_dbg, "[r4300] rom[0..15]: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
			rom[0], rom[1], rom[2], rom[3], rom[4], rom[5], rom[6], rom[7],
			rom[8], rom[9], rom[10], rom[11], rom[12], rom[13], rom[14], rom[15]);
		DLOG(_dbg);
	} else {
		DLOG("[r4300] rom is NULL!");
	}

	// copy boot code from ROM to SP_DMEM
	memcpy((char *) SP_DMEM + 0x40, rom + 0x40, 0xFC0);

	sprintf(_dbg, "[r4300] SP_DMEM[0x10..0x13]=%08X %08X %08X %08X",
		SP_DMEM[0x10/4], SP_DMEM[0x14/4], SP_DMEM[0x18/4], SP_DMEM[0x1C/4]);
	DLOG(_dbg);
	sprintf(_dbg, "[r4300] SP_DMEM[0x40..0x4C]=%08X %08X %08X %08X",
		SP_DMEM[0x40/4], SP_DMEM[0x44/4], SP_DMEM[0x48/4], SP_DMEM[0x4C/4]);
	DLOG(_dbg);
...
void cpu_deinit(void)
{
	...
	if (r4300emu != 2)
	{
		...
		for (i = 0; i < 0x100000; i++)
		{
			precomp_block* temp_block = blocks[i];
			if (temp_block)
			{
#ifdef PPC_DYNAREC

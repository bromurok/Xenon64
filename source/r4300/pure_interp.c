...
#include "../Rice_GX_Xenos/dbgswitch.h"
...
unsigned int op_usage[64] = {0};
unsigned int op_usage_spec[64] = {0};
unsigned int op_usage_fp[64] = {0};
...
static void LB()
{
	interp_addr += 4;
	address = iimmediate + irs32;
#if INTERP_PERF_MODE && INTERP_FAST_RD
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		irt = (long long)(signed char)(rdramb[(address & 0xFFFFFF)^S8]);
		return;
	}
#endif
	rdword = &irt;
...
static void LH()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		irt = (long long)(signed short)(*((unsigned short *)(rdramb + ((address & 0xFFFFFF)^S16))));
		return;
	}
#endif
	rdword = &irt;
...
static void LW()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		irt = (long long)(long)rdram[(address & 0xFFFFFF) >> 2];
		return;
	}
#endif
	rdword = &irt;
...
static void LBU()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		irt = rdramb[(address & 0xFFFFFF)^S8];
		return;
	}
#endif
	rdword = &irt;
...
static void LHU()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		irt = *((unsigned short *)(rdramb + ((address & 0xFFFFFF)^S16)));
		return;
	}
#endif
	rdword = &irt;
...
static void LWU()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		irt = (long long)(unsigned long)rdram[(address & 0xFFFFFF) >> 2];
		return;
	}
#endif
	rdword = &irt;
...
static void SB()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		rdramb[(address & 0xFFFFFF)^S8] = (unsigned char)(irt & 0xFF);
		check_memory();
		return;
	}
#endif
...
static void SH()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		*(unsigned short *)(rdramb + ((address & 0xFFFFFF)^S16)) = (unsigned short)(irt & 0xFFFF);
		check_memory();
		return;
	}
#endif
...
static void SW()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		rdram[(address & 0xFFFFFF) >> 2] = (unsigned long)(irt & 0xFFFFFFFF);
		check_memory();
		return;
	}
#endif
...
static void LWC1()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		*((long*) reg_cop1_simple[lfft]) = (long)rdram[(address & 0xFFFFFF) >> 2];
		return;
	}
#endif
...
static void LDC1()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		unsigned int _off = (address & 0xFFFFFF) >> 2;
		*((unsigned long long*) reg_cop1_double[lfft]) =
			((unsigned long long)rdram[_off] << 32) | (unsigned long long)rdram[_off + 1];
		return;
	}
#endif
...
static void LD()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		irt = ((unsigned long long)rdram[(address & 0xFFFFFF) >> 2] << 32) |
		      (unsigned long long)rdram[((address & 0xFFFFFF) >> 2) + 1];
		return;
	}
#endif
	rdword = &irt;
...
static void SWC1()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		rdram[(address & 0xFFFFFF) >> 2] = (unsigned long)(*((long*) reg_cop1_simple[lfft]));
		check_memory();
		return;
	}
#endif
...
static void SDC1()
{
	...
	if (LIKELY(address >= 0x80000000 && address < 0x80800000))
	{
		unsigned long long _val = *((unsigned long long*) reg_cop1_double[lfft]);
		unsigned int _off = (address & 0xFFFFFF) >> 2;
		rdram[_off]     = (unsigned long)(_val >> 32);
		rdram[_off + 1] = (unsigned long)(_val & 0xFFFFFFFF);
		check_memory();
		return;
	}
#endif
...
static void SD()
{
	...
	{
		unsigned int _off = (address & 0xFFFFFF) >> 2;
		rdram[_off]     = (unsigned long)(irt >> 32);
		rdram[_off + 1] = (unsigned long)(irt & 0xFFFFFFFF);
		check_memory();
		return;
	}
#endif
...
void prefetch()
{
#if INTERP_PERF_MODE && INTERP_FLAT_PREFETCH
	...
	else if ((interp_addr >= 0x80800000) && (interp_addr < 0xc0000000))
	{
		printf("execution \xff l'addresse :%x\n", (int) interp_addr);
		stop = 1;
	}
	else
	{
...
void pure_interpreter()
{
	...
	{
		char _dbg[128];
		sprintf(_dbg, "[interp] ENTERED interp_addr=%08X", (unsigned int)interp_addr);
		DLOG(_dbg);
	}
#if INTERP_PERF_MODE && INTERP_HOT_LOOP
	while (!stop)
	{
		prefetch();
#ifdef COMPARE_CORE
...
#else
	{
		...
		while (!stop)
		{
			...
			interp_ops[((op >> 26) & 0x3F)]();
			++op_usage[((op >> 26) & 0x3F)];
			if (interp_addr == 0x80000400)
			{
				sprintf(_dbg, "[interp] *** GAME ENTRY pc=%08X total=%u", (unsigned int)interp_addr, _dbg_cnt);
				DLOG(_dbg);
			}
			if ((_dbg_cnt & 0xFFFFF) == 0)
			{
				sprintf(_dbg, "[interp] milestone cnt=%u pc=%08X", _dbg_cnt, (unsigned int)interp_addr);
				DLOG(_dbg);
			}
#ifdef DBG
			PC->addr = interp_addr;
		...
		sprintf(_dbg, "[interp] STOPPED at pc=%08X total=%u", (unsigned int)interp_addr, _dbg_cnt);
		DLOG(_dbg);
	}
#endif
	PC->addr = interp_addr;

...
static __inline unsigned short extractUpper16(void* address){
	unsigned int addr = (unsigned int)address;
	return (addr>>16) + ((addr>>15)&1);
}

static __inline short extractLower16(void* address){
	unsigned int addr = (unsigned int)address;
...
static __inline int check_delaySlot(void){
...
static __inline int signExtend(int value, int size){
...
static int branch(int offset, condition cond, int link, int likely){
	...
	PowerPC_instr* preDelay;
	int delaySlot;
	preDelay = get_curr_dst();
	check_delaySlot();
	delaySlot = get_curr_dst() - preDelay;
...
int convert(void){
	...
	int result;
	mips = get_next_src();
	result = gen_ops[MIPS_GET_OPCODE(mips)](mips);
...
static int JALR(MIPS_instr mips){
	...
	// Set LR to next instruction
	int rd;
	rd = mapRegisterNew(MIPS_GET_RD(mips));
	// lis	lr, pc@ha(0)
...
static int MFC1(MIPS_instr mips){
	...
	int rt;
	fs = MIPS_GET_FS(mips);
	rt = mapRegisterNew( MIPS_GET_RT(mips) );
	flushFPR(fs);
...
static int DMFC1(MIPS_instr mips){
	...
	int addr;
	fs = MIPS_GET_FS(mips);
	rt = mapRegister64New( MIPS_GET_RT(mips) );
	addr = mapRegisterTemp();
	flushFPR(fs);
...
static int MTC1(MIPS_instr mips){
	...
	int fs;
	int addr;
	rt = mapRegister( MIPS_GET_RT(mips) );
	fs = MIPS_GET_FS(mips);
	addr = mapRegisterTemp();
	invalidateFPR(fs);
...
static int DMTC1(MIPS_instr mips){
	...
	int fs;
	int addr;
	rt = mapRegister64( MIPS_GET_RT(mips) );
	fs = MIPS_GET_FS(mips);
	addr = mapRegisterTemp();
	invalidateFPR(fs);
...
static int BC(MIPS_instr mips){
	...
	int cond;
	int likely;
	cond   = mips & 0x00010000;
	likely = mips & 0x00020000;
...
static int ADD_FP(MIPS_instr mips, int dbl){
	...
	int ft;
	int fd;
	fs = mapFPR( MIPS_GET_FS(mips), dbl );
	ft = mapFPR( MIPS_GET_FT(mips), dbl );
	fd = mapFPRNew( MIPS_GET_FD(mips), dbl );

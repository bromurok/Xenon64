...
static m64p_error write_configlist_file(void)
{
    ...
    if (configpath == NULL)
        return M64ERR_FILES;
    osal_mkdirp(configpath, 0700);

    filepath = combinepath(configpath, MUPEN64PLUS_CFG_NAME);
    if (filepath == NULL)
        return M64ERR_NO_MEMORY;

    { char dbgbuf[512]; sprintf(dbgbuf, "[config_core] Antes de fopen wb: %s\n", filepath); OutputDebugStringA(dbgbuf); }
    if (strstr(filepath, "game:\\") == filepath)
    {
        DebugMessage(M64MSG_INFO, "Skipping config write (game: is read-only).");
        free(filepath);
        return M64ERR_SUCCESS;
    }
    fPtr = fopen(filepath, "wb");
    { char dbgbuf[512]; sprintf(dbgbuf, "[config_core] Despues de fopen wb, fPtr=%p\n", (void*)fPtr); OutputDebugStringA(dbgbuf); }
    if (fPtr == NULL)
    {
...
m64p_error ConfigInit(const char *ConfigDirOverride, const char *DataDirOverride)
{
    ...
    { char dbgbuf[512]; sprintf(dbgbuf, "[config_core] Antes de fopen rb: %s\n", filepath); OutputDebugStringA(dbgbuf); }
    fPtr = fopen(filepath, "rb");
    { char dbgbuf[512]; sprintf(dbgbuf, "[config_core] Despues de fopen rb, fPtr=%p\n", (void*)fPtr); OutputDebugStringA(dbgbuf); }
    if (fPtr == NULL)
    {

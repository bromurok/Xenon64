...
static int DoGFXTask(OSTask_t *task, int sum)
{
    ...
    {
        DLOG("[RSP] DoGFXTask: calling ProcessDlistList...");
        rsp.ProcessDlistList();
        DLOG("[RSP] DoGFXTask: ProcessDlistList returned");
        taskdone();
    ...
    {
        DLOG("[RSP] DoGFXTask: GraphicsHle=0 or ProcessDlistList=NULL");
...
EXPORT unsigned int CALL doRspCycles(unsigned int Cycles)
{
    ...
    sprintf(_dbg, "[RSP] doRspCycles: type=%u flags=%u ucode_boot_size=0x%x ucode_size=0x%x run=%d",
        task->type, task->flags, task->ucode_boot_size, task->ucode_size, run_through_task);
    DLOG(_dbg);
    ...
    if (run_through_task)
    {
        ...
        sprintf(_dbg, "[RSP] task type=%u sum=0x%x PC=0x%x", task->type, sum, *rsp.SP_PC_REG);
        DLOG(_dbg);
        ...
        sprintf(_dbg, "[RSP] unknown OSTask: type=%u sum=0x%x PC=0x%x", task->type, sum, *rsp.SP_PC_REG);
        DLOG(_dbg);
    ...
    else
    {
        ...
        sprintf(_dbg, "[RSP] unknown RSP code: sum=0x%x PC=0x%x", sum, *rsp.SP_PC_REG);
        DLOG(_dbg);

...
#ifndef VM_USER_PAGE_SIZE
#define VM_USER_PAGE_SIZE 4096
#endif
...
void do_SP_Task(int delayedDP, int cycles)
{
	...
	save_pc = rsp_register.rsp_pc & ~0xFFF;
    if (SP_DMEM[0xFC0/4] == 1)
    {
        if (dpc_register.dpc_status & 0x2) // DP frozen (DK64, BC)
        {
            DLOG("[SP] DP frozen, skipping task");
            return;
        }
        DLOG("[SP] *** EXECUTING GFX TASK ***");
        ...
        DLOG("[SP] calling rsp.doRspCycles...");
        start_section(GFX_SECTION);
        rsp.doRspCycles(0xFFFFFFFF);
        end_section(GFX_SECTION);
        DLOG("[SP] rsp.doRspCycles returned");
        rsp_register.rsp_pc |= save_pc;
        new_frame();
        DLOG("[SP] RSP HLE done, new_frame called");

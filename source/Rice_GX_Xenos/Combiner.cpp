...
{
    DLOG("[CC] UpdateCombiner entered");
#ifdef _DEBUG
    if( debuggerDropDecodedMux )
    {
    ...
    {
        char _dbg3[64];
        sprintf(_dbg3, "[CC] m_pDecodedMux=%p", (void*)m_pDecodedMux);
        DLOG(_dbg3);
    }
    if (!m_pDecodedMux)
    {
        DLOG("[CC] m_pDecodedMux is NULL, allocating");
        m_pDecodedMux = new DecodedMux;
    }
    DecodedMux &m_decodedMux = *m_pDecodedMux;
    {
        char _dbg3[64];
        sprintf(_dbg3, "[CC] m_dwMux0=0x%x m_dwMux1=0x%x", m_decodedMux.m_dwMux0, m_decodedMux.m_dwMux1);
        DLOG(_dbg3);
    }
    if( m_decodedMux.m_dwMux0 != dwMux0 || m_decodedMux.m_dwMux1 != dwMux1 )
    {
        {
            char _dbg[128];
            sprintf(_dbg, "[CC] UpdateCombiner: mux0=0x%x mux1=0x%x listSize=%d", dwMux0, dwMux1, m_DecodedMuxList.size());
        ...
        if( index >= 0 )
        {
            ...
            if (_dbglog) { fprintf(_dbglog, "[CC] found cached mux\n"); fflush(_dbglog); }
        }
        else
        {
            if (_dbglog) { fprintf(_dbglog, "[CC] new mux, decoding...\n"); fflush(_dbglog); }
            m_decodedMux.Decode(dwMux0, dwMux1);
            if (_dbglog) { fprintf(_dbglog, "[CC] Decode done\n"); fflush(_dbglog); }
            ...
            m_decodedMux.Hack();
            if (_dbglog) { fprintf(_dbglog, "[CC] Hack done\n"); fflush(_dbglog); }
            ...
            if (_dbglog) { fprintf(_dbglog, "[CC] Simplify done\n"); fflush(_dbglog); }
            if( m_supportedStages>1)    
                m_decodedMux.SplitComplexStages();
            if (_dbglog) { fprintf(_dbglog, "[CC] SplitComplex done\n"); fflush(_dbglog); }
            
            m_DecodedMuxList.add(m_decodedMux.m_u64Mux, *m_pDecodedMux);
            if (_dbglog) { fprintf(_dbglog, "[CC] List add done, size=%d\n", m_DecodedMuxList.size()); fflush(_dbglog); }
#ifdef _DEBUG
            if( logCombiners ) 
            {
    ...
    DLOG("[CC] UpdateCombiner exiting");
}

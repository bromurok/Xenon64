...
CGraphicsContext * CxeDeviceBuilder::CreateGraphicsContext(void)
{
    ...
    g_pFrameBufferManager = new CxeFrameBufferManager;
    return m_pGraphicsContext;
}
...
CTexture * CxeDeviceBuilder::CreateTexture(uint32 dwWidth, uint32 dwHeight, TextureUsage usage)
{
    ...
    // m_pTexture (campo generico de CTexture) nunca se asigna en este
    // backend D3D9 - usamos GetTexture(), que esta sobreescrito para
    // devolver el puntero real (tex). Chequear m_pTexture directamente
    // siempre daba NULL y borraba la textura recien creada.
    if( txtr->GetTexture() == NULL )
    {
        delete txtr;
        TRACE0("Cannot create new texture, out of video memory");
        return NULL;

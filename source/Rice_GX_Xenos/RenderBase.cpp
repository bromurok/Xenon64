...
#include <xmmintrin.h>
void __fastcall SSEVec3Transform(int i)
{
    __m128 xmm1 = _mm_load_ps((float*)&g_vtxNonTransformed[i]);
    __m128 xmm4 = _mm_load_ps((float*)&gRSPworldProjectTransported[0]);
    __m128 xmm5 = _mm_load_ps((float*)&gRSPworldProjectTransported[4]);
    __m128 xmm6 = _mm_load_ps((float*)&gRSPworldProjectTransported[8]);
    __m128 xmm7 = _mm_load_ps((float*)&gRSPworldProjectTransported[12]);
    ...
    __m128 xmm0 = _mm_movehl_ps(_mm_setzero_ps(), xmm4);
    xmm0 = _mm_movelh_ps(xmm0, xmm5);
    xmm4 = _mm_add_ps(xmm4, xmm0);
    xmm5 = _mm_add_ps(xmm5, xmm0);

    xmm0 = _mm_shuffle_ps(xmm0, xmm4, 0x44);
    xmm4 = _mm_shuffle_ps(xmm4, xmm5, 0xe4);
    xmm5 = _mm_movehl_ps(xmm5, xmm0);
    xmm4 = _mm_shuffle_ps(xmm4, xmm4, 0x08);
    xmm5 = _mm_shuffle_ps(xmm5, xmm5, 0x0d);
    xmm4 = _mm_add_ps(xmm4, xmm5);

    __m128 xmm0b = _mm_movehl_ps(_mm_setzero_ps(), xmm6);
    xmm0b = _mm_movelh_ps(xmm0b, xmm7);
    xmm6 = _mm_add_ps(xmm6, xmm0b);
    xmm7 = _mm_add_ps(xmm7, xmm0b);
    xmm0b = _mm_shuffle_ps(xmm0b, xmm6, 0x44);
    xmm6 = _mm_shuffle_ps(xmm6, xmm7, 0xe4);
    xmm7 = _mm_movehl_ps(xmm7, xmm0b);
    xmm6 = _mm_shuffle_ps(xmm6, xmm6, 0x08);
    xmm7 = _mm_shuffle_ps(xmm7, xmm7, 0x0d);
    xmm6 = _mm_add_ps(xmm6, xmm7);

    xmm4 = _mm_movelh_ps(xmm4, xmm6);
    _mm_store_ps((float*)&g_vtxTransformed[i], xmm4);

    __m128 xmmw = _mm_shuffle_ps(xmm4, xmm4, 0xff);
    __m128 xmmDiv = _mm_div_ps(xmm4, xmmw);
    __m128 xmmHi = _mm_movehl_ps(xmmDiv, xmmDiv);
    xmmHi = _mm_shuffle_ps(xmmHi, xmmHi, 0xe8);
    xmmDiv = _mm_movelh_ps(xmmDiv, xmmHi);
    _mm_store_ps((float*)&g_vecProjected[i], xmmDiv);
}
void __fastcall SSEVec3TransformDKR(XVECTOR4 &pOut, const XVECTOR4 &pV)
{
    __m128 xmm1 = _mm_load_ps((float*)&pV);
    __m128 xmm4 = _mm_load_ps((float*)&dkrMatrixTransposed[0]);
    __m128 xmm5 = _mm_load_ps((float*)&dkrMatrixTransposed[4]);
    __m128 xmm6 = _mm_load_ps((float*)&dkrMatrixTransposed[8]);
    __m128 xmm7 = _mm_load_ps((float*)&dkrMatrixTransposed[12]);

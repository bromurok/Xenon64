...
void DLParser_TexRect(Gfx *gfx)
{
...

    {
        ...
        trCount++;
        if (trCount <= 500 && (trCount <= 40 || trCount % 20 == 0))
        {
            char dbg[256];
            sprintf(dbg, "[texrect] #%d XL=%d YL=%d XH=%d YH=%d scissor(L=%d T=%d R=%d B=%d)",
                trCount, dwXL, dwYL, dwXH, dwYH,
                gRDP.scissor.left, gRDP.scissor.top, gRDP.scissor.right, gRDP.scissor.bottom);
            DLOG(dbg);
        }
    }

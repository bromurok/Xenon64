...
#else //__GX__
DWORD g_maxTextureMemUsage = (32*1024*1024);
DWORD g_amountToFree = (2*1024*1024);
...
void CTextureManager::RemoveTexture(TxtrCacheEntry * pEntry)
{
    ...
    {
        char _dbg[256];
        sprintf(_dbg, "[TexConv32] pF=NULL! Cannot decompress %s/%dbpp fmt=%d size=%d",
            pszImgFormat[pEntry->ti.Format], pnImgSize[pEntry->ti.Size],
            (int)pEntry->ti.Format, (int)pEntry->ti.Size);
        DLOG(_dbg);
...
#endif //__GX__
    {
        static int texGetIdx = 0;
        texGetIdx++;
        char _dbg[256];
        sprintf(_dbg, "[TexGet] #%d addr=0x%08X fmt=%d size=%d w=%d h=%d load=%d fromTMEM=%d",
            texGetIdx,
            pgti->Address,
            (int)pgti->Format,
            (int)pgti->Size,
            (int)pgti->WidthToCreate,
            (int)pgti->HeightToCreate,
            (int)pgti->WidthToLoad,
            (int)fromTMEM);
        DLOG(_dbg);
    }
    ...
    if( (frameBufferOptions.bCheckRenderTextures&&!frameBufferOptions.bWriteBackBufToRDRAM) || (frameBufferOptions.bCheckBackBufs&&!frameBufferOptions.bWriteBackBufToRDRAM) )
    {
        ...
        if( txtBufIdxToLoadFrom >= 0 )
        {
                ...
                && info.CI_Info.dwSize == pgti->Size )
            {
                ...
                DLOG("[TexGet]  → RETURN from render texture buffer");
...
//                loadFromTextureBuffer = true;
                ...
                {
                    info.txtEntry.ti = *pgti;
                    DLOG("[TexGet]  → RETURN from back buffer");
                    return &info.txtEntry;
                }
            }
        }
    ...
    if (pEntry && doCRCCheck )
    {
            ...
            (!loadFromTextureBuffer || gRenderTextureInfos[txtBufIdxToLoadFrom].updateAtFrame < pEntry->FrameLastUsed ) )
        {
            ...
            DLOG("[TexGet]  → RETURN cached (CRC hit)");
    ...
    if( pEntry->pTexture == NULL )
    {
        // La textura nunca se creo correctamente (CreateTexture fallo en el
        // device builder). Evitar el crash y devolver NULL en vez de
        // acceder a un puntero invalido.
        DLOG("[TexGet]  → RETURN NULL (pTexture==NULL after create)");
        _VIDEO_DisplayTemporaryMessage("pEntry->pTexture es NULL, abortando GetTexture");
        return NULL;
    }
    ...
    try 
    {
		...
		DEBUG_print(txtbuffer,DBG_RICE); 
            ...
            {
                static int dwTypeCount = 0;
                dwTypeCount++;
                if (dwTypeCount <= 60 || (dwTypeCount % 300 == 0))
                {
                    if (_dbglog) { fprintf(_dbglog, "[dwtype] #%d dwType=%d UNKNOWN=%d A8R8G8B8=%d tex=%p\n", dwTypeCount, (int)dwType, (int)TEXTURE_FMT_UNKNOWN, (int)TEXTURE_FMT_A8R8G8B8, (void*)pEntry->pTexture); fflush(_dbglog); }
                }
            }
            SAFE_DELETE(pEntry->pEnhancedTexture);
...
#endif //!__GX__
            {
                ...
                else
                {
                    ...
                    {
                        char _dbg[256];
                        sprintf(_dbg, "[TexConv] 32bit fmt=%d size=%d w=%d h=%d addr=0x%08X pF=ConvertTexture",
                            (int)pEntry->ti.Format, (int)pEntry->ti.Size,
                            (int)pEntry->ti.WidthToLoad, (int)pEntry->ti.HeightToLoad,
                            pEntry->ti.Address);
                        DLOG(_dbg);
                    ...
                    {
                        char _dbg[256];
                        sprintf(_dbg, "[TexConv] 16bit fmt=%d size=%d w=%d h=%d addr=0x%08X pF=ConvertTexture_16",
                            (int)pEntry->ti.Format, (int)pEntry->ti.Size,
                            (int)pEntry->ti.WidthToLoad, (int)pEntry->ti.HeightToLoad,
                            pEntry->ti.Address);
                        DLOG(_dbg);
    ...
    catch (...)
    {
        ...
        DLOG("[TexGet]  → RETURN NULL (exception in decompression)");
    ...
    {
        char _dbg[256];
        sprintf(_dbg, "[TexGet]  → RETURN OK tex=%p fmt=%d size=%d w=%d h=%d addr=0x%08X",
            (void*)pEntry->pTexture, (int)pEntry->ti.Format, (int)pEntry->ti.Size,
            (int)pEntry->ti.WidthToCreate, (int)pEntry->ti.HeightToCreate,
            pEntry->ti.Address);
        DLOG(_dbg);
    }
    return pEntry;
...
void CTextureManager::ConvertTexture(TxtrCacheEntry * pEntry, bool fromTMEM)
{
    ...
    {
        static int convLogCount = 0;
        convLogCount++;
        char _dbg[256];
        sprintf(_dbg, "[TexConv32] #%d fmt=%d size=%d pF=%p bSw=%d addr=0x%08X",
            convLogCount, (int)pEntry->ti.Format, (int)pEntry->ti.Size,
            (void*)pF, (int)pEntry->ti.bSwapped, pEntry->ti.Address);
        DLOG(_dbg);
    }
    if( pF )
    {
...

        {
            ...
            {
                uint32 *pixels = (uint32*)di.lpSurface;
                uint32 p0 = pixels[0];
                uint32 p1 = (di.dwWidth > 1) ? pixels[1] : 0;
                uint32 pc = pixels[(di.dwHeight/2) * (di.lPitch/4) + di.dwWidth/2];
                char _dbg[256];
                sprintf(_dbg, "[TexSample32] w=%d h=%d pitch=%d p0=0x%08X p1=0x%08X pc=0x%08X",
                    (int)di.dwWidth, (int)di.dwHeight, (int)di.lPitch,
                    p0, p1, pc);
                DLOG(_dbg);
...
void CTextureManager::ConvertTexture_16(TxtrCacheEntry * pEntry, bool fromTMEM)
{
    ...
    {
        static int conv16Count = 0;
        conv16Count++;
        char _dbg[256];
        sprintf(_dbg, "[TexConv16] #%d fmt=%d size=%d pF=%p bSw=%d addr=0x%08X",
            conv16Count, (int)pEntry->ti.Format, (int)pEntry->ti.Size,
            (void*)pF, (int)pEntry->ti.bSwapped, pEntry->ti.Address);
        DLOG(_dbg);
    }
    if( pF )
    {
...

        {
            ...
            if (pEntry->pTexture->StartUpdate(&di))
            {
                ...
                uint32 pc = pixels[(di.dwHeight/2) * (di.lPitch/4) + di.dwWidth/2];
                char _dbg[256];
                sprintf(_dbg, "[TexSample16] w=%d h=%d pitch=%d p0=0x%08X p1=0x%08X pc=0x%08X",
                    (int)di.dwWidth, (int)di.dwHeight, (int)di.lPitch,
                    p0, p1, pc);
                DLOG(_dbg);
                pEntry->pTexture->EndUpdate(&di);
    ...
    {
        char _dbg[256];
        sprintf(_dbg, "[TexConv16] pF=NULL! Cannot decompress %s/%dbpp fmt=%d size=%d",
            pszImgFormat[pEntry->ti.Format], pnImgSize[pEntry->ti.Size],
            (int)pEntry->ti.Format, (int)pEntry->ti.Size);
        DLOG(_dbg);
        TRACE2("ConvertTexture: Unable to decompress %s/%dbpp", pszImgFormat[pEntry->ti.Format], pnImgSize[pEntry->ti.Size]);

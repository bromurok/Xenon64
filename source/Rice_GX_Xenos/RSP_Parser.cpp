...
void DLParser_Process(OSTask * pTask)
{
...
#ifdef XENON
    status.gRDPTime = GetTickCount() * 1000;
#else
    struct timeval tv;
    gettimeofday(&tv, 0);
    ...
    if ( lastUcodeInfo.ucStart != (uint32)(pTask->t.ucode) )
    {
        ...
        {
            char _dbg[256];
            sprintf(_dbg, "[GFX] DLParser_CheckUcode: returned ucode=%u (ucStart=0x%x ucDStart=0x%x)", ucode, (uint32)pTask->t.ucode, (uint32)pTask->t.ucode_data);
            DLOG(_dbg);
        }
    ...
    {
        char _dbg[256];
        uint32 pc = gDlistStack[gDlistStackPointer].pc;
        uint32 cmd0 = (pc < g_dwRamSize*2) ? g_pRDRAMu32[pc>>2] : 0;
        uint32 cmd1 = (pc < g_dwRamSize*2) ? g_pRDRAMu32[(pc>>2)+1] : 0;
        sprintf(_dbg, "[GFX] DLParser_Process: ucode=0x%x data_ptr=0x%x data_size=0x%x pc_first_cmd=0x%x cmd=0x%08x_%08x g_dwRamSize=0x%x",
            (uint32)pTask->t.ucode, (uint32)pTask->t.data_ptr, (uint32)pTask->t.data_size, pc, cmd0, cmd1, g_dwRamSize);
        DLOG(_dbg);
    }
...
//    if (status.gRDPTime - status.lastPurgeTimeTime > 5000)
    ...
    SetVIScales();
    DLOG("[GFX] DLParser_Process: before RenderReset");
    CRender::g_pRender->RenderReset();
    DLOG("[GFX] DLParser_Process: before BeginRendering");
    CRender::g_pRender->BeginRendering();
    DLOG("[GFX] DLParser_Process: before SetViewport");
    ...
    try
    {
        uint32 dlLoopCount = 0;
        // The main loop
        while( gDlistStackPointer >= 0 )
        {
            dlLoopCount++;
            if (dlLoopCount <= 70 || (dlLoopCount % 100) == 0)
            {
                char _dbg[256];
                uint32 curPC = gDlistStack[gDlistStackPointer].pc;
                uint32 w0 = (curPC < g_dwRamSize*2) ? g_pRDRAMu32[curPC>>2] : 0xDEAD;
                uint32 w1 = (curPC < g_dwRamSize*2) ? g_pRDRAMu32[(curPC>>2)+1] : 0xDEAD;
                sprintf(_dbg, "[GFX] dlist cmd#%u pc=0x%x w0=0x%08x w1=0x%08x op=0x%02x stack=%d cnt=%d",
                    dlLoopCount, curPC, w0, w1, w0>>24, gDlistStackPointer,
                    gDlistStack[gDlistStackPointer].countdown);
                DLOG(_dbg);
            }
            if (dlLoopCount > 500000)
            {
                char _dbg[256];
                sprintf(_dbg, "[GFX] *** DLIST HANG DETECTED after %u commands ***", dlLoopCount);
                DLOG(_dbg);
                break;
            }
#ifdef _DEBUG
            ...
            {
                char _dbg[64];
                sprintf(_dbg, "[GFX] cmd#%u handler returned", dlLoopCount);
                DLOG(_dbg);
            }
        ...
        DLOG("[GFX] DLParser_Process: while loop exited normally");

    }
    catch(...)
    {
        DLOG("[GFX] DLParser_Process: EXCEPTION in while loop");
        TriggerDPInterrupt();
    }

    {
        char _dbg[128];
        sprintf(_dbg, "[GFX] DLParser_Process: loop ended, gDlistStackPointer=%d", gDlistStackPointer);
        DLOG(_dbg);
    }
...
{
    { char __buf[256]; _snprintf(__buf, sizeof(__buf), "[BUG_HUNT] NOIMPL: %s (w0=%08X w1=%08X)", op, word0, word1); DLOG(__buf); }
#ifdef _DEBUG
    if( logWarning )
    {
        TRACE0("Stack Trace");
...
void DLParser_SetCombine(Gfx *gfx)
{
    ...
    char _dbg[128];
    sprintf(_dbg, "[GFX] DLParser_SetCombine: mux0=0x%08x mux1=0x%08x", dwMux0, dwMux1);
    DLOG(_dbg);
    CRender::g_pRender->SetMux(dwMux0, dwMux1);
    DLOG("[GFX] DLParser_SetCombine: SetMux returned");
}

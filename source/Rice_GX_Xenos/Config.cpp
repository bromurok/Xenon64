...
void GenerateFrameBufferOptions(void)
{
    ...
    { char __buf[256]; _snprintf(__buf, sizeof(__buf),
        "[CFG] FBFlags: CIInfo=%d ChkBack=%d WrBack=%d LdBack=%d SupRT=%d ChkRT=%d WrRT=%d LdRT=%d Ignore=%d CPUw=%d CPUr=%d EachFrame=%d",
        frameBufferOptions.bUpdateCIInfo, frameBufferOptions.bCheckBackBufs,
        frameBufferOptions.bWriteBackBufToRDRAM, frameBufferOptions.bLoadBackBufFromRDRAM,
        frameBufferOptions.bSupportRenderTextures, frameBufferOptions.bCheckRenderTextures,
        frameBufferOptions.bRenderTextureWriteBack, frameBufferOptions.bLoadRDRAMIntoRenderTexture,
        frameBufferOptions.bIgnore, frameBufferOptions.bProcessCPUWrite,
        frameBufferOptions.bProcessCPURead, frameBufferOptions.bAtEachFrameUpdate); DLOG(__buf); }
}
...
void WriteConfiguration(void)
{
   ...
   if (!f)
    ...
    windowSetting.uFullScreenDisplayWidth=1280;
    windowSetting.uFullScreenDisplayHeight=720;
     }
   else
     fclose(f);
...
void GenerateCurrentRomOptions()
{
    ...
    else if ((strstr((char*)g_curRomInfo.szGameName, "ZELDA") != 0))
    {
        ...
        DLOG("[BUG_HUNT] Zelda hack activated!");
    }
    ...
    { char __buf[256]; _snprintf(__buf, sizeof(__buf), "[CFG] FrameBuffer=%d RenderToTex=%d DefaultFB=%d DefaultRTT=%d dwFBOption=%d dwRTTOption=%d",
        currentRomOptions.N64FrameBufferEmuType, currentRomOptions.N64RenderToTextureEmuType,
        defaultRomOptions.N64FrameBufferEmuType, defaultRomOptions.N64RenderToTextureEmuType,
        g_curRomInfo.dwFrameBufferOption, g_curRomInfo.dwRenderToTextureOption); DLOG(__buf); }

    if( currentRomOptions.N64FrameBufferEmuType == FRM_BUF_NONE && currentRomOptions.N64RenderToTextureEmuType == TXT_BUF_NONE )
    {
        if( options.enableHackForGames == HACK_FOR_ZELDA || options.enableHackForGames == HACK_FOR_ZELDA_MM )
        {
            currentRomOptions.N64FrameBufferEmuType = FRM_BUF_BASIC;
            currentRomOptions.N64RenderToTextureEmuType = TXT_BUF_NORMAL;
            { DLOG("[CFG] FORCE: Zelda — FB=BASIC, RTT=NORMAL (no ini match)"); }
        }
        else if( options.enableHackForGames == HACK_FOR_MARIO_KART )
        {
            currentRomOptions.N64FrameBufferEmuType = FRM_BUF_BASIC;
            currentRomOptions.N64RenderToTextureEmuType = TXT_BUF_NORMAL;
            { DLOG("[CFG] FORCE: MarioKart — FB=BASIC, RTT=NORMAL (no ini match)"); }
        }
    }
...
BOOL ReadIniFile()
{
    ...
    { char __buf[256]; _snprintf(__buf, sizeof(__buf), "[INI] Opening: %s", filename); DLOG(__buf); }
    inifile.open(filename);

    if (inifile.fail())
    {
        { DLOG("[INI] FAILED to open ini file!"); }
        return FALSE;
    }

    { DLOG("[INI] ini file opened OK"); }
...
int FindIniEntry(uint32 dwCRC1, uint32 dwCRC2, uint8 nCountryID, char* szName)
{
...
#if defined(_BIG_ENDIAN) && !defined(XENON)
	// Fix the CRC-ID for Big Endian (little-endian only: swap bytes to match ini format)
    CHAR szCRCtmp[50+1];
    ...
    { char __buf[256]; _snprintf(__buf, sizeof(__buf), "[INI] CRC1=%08X CRC2=%08X Country=%02X Generated=%s Sections=%d",
        dwCRC1, dwCRC2, nCountryID, szCRC, (int)IniSections.size()); DLOG(__buf); }

    for (i = 0; i < IniSections.size(); i++)
    {
        if (strncasecmp((char*)szCRC, IniSections[i].crccheck,16) == 0) //gli: HACK: don't check Country ID
        {
            { char __buf[256]; _snprintf(__buf, sizeof(__buf), "[INI] MATCH! idx=%d name='%s' CRC=%s FB=%d RTT=%d",
                i, IniSections[i].name, szCRC, IniSections[i].dwFrameBufferOption, IniSections[i].dwRenderToTextureOption); DLOG(__buf); }
            return i;
        }
    }

    { char __buf[256]; _snprintf(__buf, sizeof(__buf), "[INI] NO MATCH for %s, creating new entry", szCRC); DLOG(__buf); }

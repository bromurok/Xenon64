...
void EnsureGraphicsDevice(void)
{
    ...
    CGraphicsContext::Get()->Initialize(0, 0, 640, 480, TRUE);
...
//	_break();
    ...
    { char __buf[256]; _snprintf(__buf, sizeof(__buf), "[BUG_HUNT] szGameName=%s", g_curRomInfo.szGameName); DLOG(__buf); }
    ...
    InitExternalTextures();
	 DLOG("[BUG_HUNT] InitExternalTextures DONE");
    {
        bool res;
        if (CGraphicsContext::g_pGraphicsContext == NULL)
        {
          DLOG("[BUG_HUNT] StartVideo: contexto NULL, voy a crear/inicializar");
#ifdef __GX__
            CDeviceBuilder::SelectDeviceType(OGL_DEVICE);
#endif //__GX__
#ifdef XENON
            CDeviceBuilder::SelectDeviceType(OGL_DEVICE);
#endif
			DLOG("[BUG_HUNT] antes de CreateGraphicsContext");
            CDeviceBuilder::GetBuilder()->CreateGraphicsContext();
			  DLOG("[BUG_HUNT] CreateGraphicsContext DONE");
            CGraphicsContext::InitWindowInfo();
			 DLOG("[BUG_HUNT] InitWindowInfo DONE");
            windowSetting.bDisplayFullscreen = FALSE;
			DLOG("[BUG_HUNT] antes de Initialize");
            res = CGraphicsContext::Get()->Initialize(0, 0, 640, 480, TRUE);
			DLOG("[BUG_HUNT] Initialize DONE");
            if( !res )
            {
                ErrorMsg("Error to start video");
            }
			DLOG("[BUG_HUNT] antes de CreateRender");
            CDeviceBuilder::GetBuilder()->CreateRender();
			DLOG("[BUG_HUNT] CreateRender DONE");
            CRender::GetRender()->Initialize();
			DLOG("[BUG_HUNT] CRender::Initialize DONE");
        }
        else
        {
			DLOG("[BUG_HUNT] StartVideo: contexto ya existia, salteando init");
            res = true;
        }
        if( res )
        {
			DLOG("[BUG_HUNT] antes de DLParser_Init");
            DLParser_Init();
			DLOG("[BUG_HUNT] DLParser_Init DONE");
        }
        status.bGameIsRunning = true;
    }
    g_CritialSection.Unlock();
	DLOG("[BUG_HUNT] StartVideo COMPLETO");
}
...
{
	DLOG("[BUG_HUNT] RomOpen ENTRADA");
#ifdef DEBUGON
//	_break();
#endif

   InitConfiguration();
   DLOG("[BUG_HUNT] InitConfiguration DONE");
...
# endif
   ...
   g_dwRamSize = 0x800000;
    ...
    videoThread = CreateThread( NULL, 128*1024, VideoThreadProc, NULL, NULL, &threadID );

#else
	DLOG("[BUG_HUNT] antes de StartVideo() desde RomOpen");
    StartVideo();
#endif
...
{
    char _dbg[256];
    sprintf(_dbg, "[UpdateScreen] gDlistCount=%u gFrameCount=%u", status.gDlistCount, status.gFrameCount);
    DLOG(_dbg);
...
{
    g_CritialSection.Lock();
    DLOG("[GFX] ProcessDListStep2: entered");
    if( status.toShowCFB )
    {
        CRender::GetRender()->DrawFrameBuffer(true);
    ...
    {
        char _dbg[128];
        sprintf(_dbg, "[GFX] ProcessDListStep2: calling DLParser_Process, g_pRender=%p", CRender::g_pRender);
        DLOG(_dbg);
        DLParser_Process((OSTask *)(g_GraphicsInfo.DMEM + 0x0FC0));
        DLOG("[GFX] ProcessDListStep2: DLParser_Process returned");
    }
    catch (...)
    {
        DLOG("[GFX] ProcessDListStep2: EXCEPTION in DLParser_Process");
        TriggerDPInterrupt();
        TriggerSPInterrupt();
    }

    g_CritialSection.Unlock();
    DLOG("[GFX] ProcessDListStep2: done");
}   

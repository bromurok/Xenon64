...
void FindAllTexturesFromFolder(char *foldername, CSortedList<uint64,ExtTxtrInfo> &infos, bool extraCheck, bool bRecursive)
{
    ...
    char searchPattern[_MAX_PATH];
    strcpy(searchPattern, foldername);
    strcat(searchPattern, "*");

    WIN32_FIND_DATA findData;
    HANDLE dir = FindFirstFile(searchPattern, &findData);
    ...
    while( hasEntry )
    {
        ...
        {
            hasEntry = (FindNextFile(dir, &findData) != 0);
            continue;
        }

        strcpy(texturefilename, foldername);
        strcat(texturefilename, findData.cFileName);
        ...
        if( strstr(findData.cFileName,(const char*)g_curRomInfo.szGameName) == 0 )
        {
            hasEntry = (FindNextFile(dir, &findData) != 0);
            continue;
        }
        ...
        if( strcasecmp(right(findData.cFileName,7), "_ci.bmp") == 0 )
        {
            if( GetImageInfoFromFile(texturefilename, &imgInfo) != 0)
            {
                TRACE1("Cannot get image info for file: %s", findData.cFileName);
                hasEntry = (FindNextFile(dir, &findData) != 0);
                continue;
            }
        ...
        else if( strcasecmp(right(findData.cFileName,13), "_ciByRGBA.png") == 0 )
        {
            if( GetImageInfoFromFile(texturefilename, &imgInfo) != 0 )
            {
                TRACE1("Cannot get image info for file: %s", findData.cFileName);
        ...
        else if( strcasecmp(right(findData.cFileName,16), "_allciByRGBA.png") == 0 )
        {
            if( GetImageInfoFromFile(texturefilename, &imgInfo) != 0 )
            {
                TRACE1("Cannot get image info for file: %s", findData.cFileName);
        ...
        else if( strcasecmp(right(findData.cFileName,8), "_rgb.png") == 0 )
        {
            if( GetImageInfoFromFile(texturefilename, &imgInfo) != 0 )
            {
                TRACE1("Cannot get image info for file: %s", findData.cFileName);
        ...
        else if( strcasecmp(right(findData.cFileName,8), "_all.png") == 0 )
        {
            if( GetImageInfoFromFile(texturefilename, &imgInfo) != 0 )
            {
                TRACE1("Cannot get image info for file: %s", findData.cFileName);
        ...
        if( type != NO_TEXTURE )
        {
            ...
            strcpy(texturefilename, findData.cFileName);

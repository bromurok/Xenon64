...
void FrameBufferManager::Set_CI_addr(SetImgInfo &newCI)
{
    ...
    { char __buf[256]; _snprintf(__buf, sizeof(__buf), "[CI] addr=%08X fmt=%d size=%d w=%d ZI=%08X isTexBuf=%d rttType=%d handleRT=%d",
        newCI.dwAddr, newCI.dwFormat, newCI.dwSize, newCI.dwWidth, g_ZI.dwAddr,
        status.bN64IsDrawingTextureBuffer, currentRomOptions.N64RenderToTextureEmuType,
        status.bHandleN64RenderTexture); DLOG(__buf); }
...
#ifdef XENON
void CxeFrameBufferManager::CopyBackBufferToRenderTexture(int idx, RecentCIInfo &ciInfo, RECT* pRect)
{
    if (idx < 0 || gRenderTextureInfos[idx].pRenderTexture == NULL)
        return;

    CxeRenderTexture* pRT = (CxeRenderTexture*)gRenderTextureInfos[idx].pRenderTexture;
    IDirect3DSurface9* pDstSurface = pRT->GetRTSurface();
    if (!pDstSurface || !g_pd3dDevice)
        return;

    IDirect3DTexture9* pDstTex = pRT->GetD3DTexture();
    if (!pDstTex)
        return;
    D3DRECT srcRect;
    D3DRECT* pSrcRectPtr = NULL;
    if (pRect)
    {
        srcRect.x1 = pRect->left; srcRect.y1 = pRect->top;
        srcRect.x2 = pRect->right; srcRect.y2 = pRect->bottom;
        pSrcRectPtr = &srcRect;
    }
    g_pd3dDevice->Resolve(D3DRESOLVE_RENDERTARGET0, pSrcRectPtr, pDstTex, NULL, 0, 0, NULL, 0.0f, 0, NULL);
    char dbg[256];
    sprintf(dbg, "[d3d9] CopyBackBufferToRenderTexture idx=%d (Resolve)", idx);
    DLOG(dbg);
}
void CxeFrameBufferManager::StoreBackBufferToRDRAM(uint32 addr, uint32 fmt, uint32 siz, uint32 width,
    uint32 height, uint32 bufWidth, uint32 bufHeight, uint32 startaddr,
    uint32 memsize, uint32 pitch, SURFFORMAT surf_fmt)
{
    if (!g_pd3dDevice || addr == 0 || width == 0 || height == 0)
        return;

    // Obtener el back buffer actual
    IDirect3DSurface9* pBackBuffer = NULL;
    HRESULT hrGet = g_pd3dDevice->GetRenderTarget(0, &pBackBuffer);
    if (FAILED(hrGet) || !pBackBuffer)
        return;

    // Crear staging texture en system memory (estatica, reutilizada)
    static IDirect3DTexture9* s_pBBStaging = NULL;
    static int s_bbW = 0, s_bbH = 0;
    if (!s_pBBStaging || s_bbW != (int)bufWidth || s_bbH != (int)bufHeight)
    {
        if (s_pBBStaging) { s_pBBStaging->Release(); s_pBBStaging = NULL; }
        HRESULT hr = g_pd3dDevice->CreateTexture(bufWidth, bufHeight, 1, 0,
            D3DFMT_LIN_A8R8G8B8, D3DPOOL_SYSTEMMEM, &s_pBBStaging, NULL);
    ...
    s_pBBStaging->GetSurfaceLevel(0, &pStagingSurface);
    if (!pStagingSurface) { pBackBuffer->Release(); return; }

    // Usar D3DXLoadSurfaceFromSurface para manejar conversion de formato
    RECT srcRect = {0, 0, (LONG)bufWidth, (LONG)bufHeight};
    RECT dstRect = {0, 0, (LONG)bufWidth, (LONG)bufHeight};
    HRESULT hrCopy = D3DXLoadSurfaceFromSurface(pStagingSurface, NULL, &dstRect,
        pBackBuffer, NULL, &srcRect, D3DX_FILTER_POINT, 0xFF000000);
    pBackBuffer->Release();
    if (FAILED(hrCopy)) { pStagingSurface->Release(); return; }

    // Lock y leer
    D3DLOCKED_RECT lockedRect;
    HRESULT hrLock = s_pBBStaging->LockRect(0, &lockedRect, NULL, D3DLOCK_READONLY);
    if (FAILED(hrLock)) { pStagingSurface->Release(); return; }
    ...
    if (siz == TXT_SIZE_16b)
    {
        ...
        uint32 n64Pitch = (pitch > 0) ? pitch : width;
        for (uint32 y = 0; y < height; y++)
        {
